...
export default function PortalHearingAidsPage() {
  ...
  const left = aids.filter((a) => a.ear === "left");
  const right = aids.filter((a) => a.ear === "right");
    ...
    <div className="max-w-5xl">
      ...
      {aids.length === 0 ? (
        ...
        <div className="space-y-8">
          {left.length > 0 && <EarSection label="Left Ear" aids={left} isWarrantyExpired={isWarrantyExpired} />}
          {right.length > 0 && <EarSection label="Right Ear" aids={right} isWarrantyExpired={isWarrantyExpired} />}
        </div>
      )}
    </div>
  );
...
function EarSection({
  ...
  label: string;
  aids: HearingAid[];
  isWarrantyExpired: (date: string | null) => boolean;
}) {
  return (
    <section>
